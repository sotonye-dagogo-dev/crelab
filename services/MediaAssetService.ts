...
export class MediaAssetService {
  ...
  /** Every Cloudinary public id currently referenced by profiles/portfolios. */
  static async loadReferencedPublicIds(): Promise<Set<string>> {
    ...
    return collectReferencedPublicIds(providerRows, portfolioRows);
  }

  static async isReferenced(publicId: string): Promise<boolean> {
    const [providerHits, portfolioHits] = await Promise.all([
      db
    ...
    return providerHits.length > 0 || portfolioHits.length > 0;
  }
