...
/**
 * Generic visual blocks builder — the "no HTML needed" editor used by both the
 * email template builder and the blog page builder. Consumers pass the current
 * blocks plus the list of `{{variable}}` tokens they want exposed (blog pages
 * typically pass none).
 */
    ...
    <div className="flex flex-col gap-3">
      {blocks.length === 0 && (
        <div className="rounded-[8px] border border-dashed border-[var(--color-border-mid)] p-6 text-center text-[13px] text-[var(--color-text-tertiary)]">
          No blocks yet — add a heading, paragraph, image, button or list to build this content visually.
        </div>
      )}
      ...
      {blocks.map((block, index) => (
        ...
        >
          ...
          {block.type === "button" && (
            <div className="flex flex-col gap-2">
              <input className={inputClass} placeholder="Button text (e.g. Explore Creators)" value={block.text} onChange={(e) => updateBlock(index, { ...block, text: e.target.value })} />
              <input className={inputClass} placeholder="Button link (e.g. /explore or {{exploreUrl}})" value={block.url} onChange={(e) => updateBlock(index, { ...block, url: e.target.value })} />
            </div>
          )}
          {block.type === "image" && (
            <div className="flex flex-col gap-2">
              <input className={inputClass} placeholder="Image URL (e.g. /primary-logo.png)" value={block.url} onChange={(e) => updateBlock(index, { ...block, url: e.target.value })} />
